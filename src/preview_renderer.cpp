...
private:
    ...
    void UpdateDeferredLightBuffer()
    {
        ...
        // Phase 1s intentionally disables the guessed shadow lookup. We have
        // APE's t54 maps but not t40 gSunShadowTree, so enabling this flag would
        // make a non-APE projection authoritative over otherwise captured light.
        data.apeShadowParams = {1.0f / 1024.0f, 2.0f / 65535.0f, 0.0f, 0.0040f};
        D3D11_MAPPED_SUBRESOURCE mapped{};
    ...
    void RenderDeferredMaterialPreview()
    {
        ...
        // Phase 1s direct-sun truth baseline: do not feed the compositor the
        // guessed single-camera shadow map. APE actually chooses among three
        // sun-shadow layers through gSunShadowTree (t40), and that structured
        // buffer was not present in the resource dump. The approximation could
        // classify the whole sphere as shadow and completely hide captured sun
        // lighting. Keep the shadow resources/code for the later t40 port, but
        // leave them out of the active APE Match path for now.
...
PS_OUT ps_main(VS_OUT i)
{
    ...
    return o;
        ...
        {
            error = L"Built-in APE deferred lighting shader resource is missing.";
            return false;
        }
        const QByteArray deferredLightPsBytes = deferredLightResource.readAll();
        const std::string deferredLightPsSourceStorage(
            deferredLightPsBytes.constData(), static_cast<size_t>(deferredLightPsBytes.size()));
        if (deferredLightPsSourceStorage.empty())
        {
